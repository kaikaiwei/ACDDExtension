...
fprintf(stdout, "***ACDD*** ACDD on androi rid is null 1");
        if (accessor != NULL) {
            accessor->reportError(accessorCookie, "No resource found that matches the given name");
        }
        return false;
...
#ifdef ACDD
if (packageId == 0 || (packageId == ACDDAaptConfig::getInstance()->getPackageGroupId() && mAppAsLib)) {
#else
